 ...
 * @author LuisT23
 */
public class EdificioVehiculo1 extends Edificios implements Edificio{
    public ArrayList<Vehiculo> vehiculos;
    public int tasaRecurso;
    
    public EdificioVehiculo1(){
        vehiculos=new ArrayList<>();
    }

    public EdificioVehiculo1(edificioType type) {
        this.type=type;
        vehiculos=new ArrayList<>();
    }

    @Override
    public void recibirDaño(int daño) { vida -= daño;}
    public void generarTropa(MiliciaType type) {}
    @Override
    ...
    public void generarTropa(Razas raza,Fortaleza fortaleza,CentroDeMando th) {
        ...
        Vehiculo e=factory.getVehiculo(VehiculoType.VEHICULO1, raza, fortaleza);
        if(th.elaborar(e.getCosto()[0], e.getCosto()[1], e.getCosto()[2])){
           vehiculos.add(e);
           th.elaborar(e.getCosto());
...
@Override
    ...
    public String costoToString() {
        return edificioType.VEHICULO1+"   costo{ "+edificioType.RECURSO1+": "
    ...
    public void setFase(ArrayList<String> mensajes) {
        ...
        if(tiempoespera==0){
            detectable=true;
            mensajes.add(edificioType.VEHICULO1+" "+raza+" construido");
        }
    }else{
         for (Vehiculo e:vehiculos){
             e.setFase(mensajes);}}
